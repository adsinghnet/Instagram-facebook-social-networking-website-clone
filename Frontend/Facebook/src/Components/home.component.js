    ...
    const GeneratePost = ({ post }) => {
        return <div key={post._id} className="box box-shadow margin-top-bottom container-sm px-4 w-50">
            ...
            <div className="post-section">
                {!likeInfo[post._id]
                    ? <span className="to-like-icon cp" onClick={() => { LikePost(post._id) }}><img src={LikeIcon} alt="Like Icon" /></span>
                    : <span className="to-like-icon cp" onClick={() => { UnlikePost(post._id) }}><img src={HeartIcon} alt="Unlike Icon" /></span>}
            ...
            <p className="box-text">Posted {moment(post.createdAt).fromNow()} by <NavLink to={`/u/${post.creator}`} className="link text-primary">{post.creator}</NavLink></p>
        ...
        <div className="container">
            <div className="home">
                {posts.length !== 0 ? (
                    ...
                    : <h1>Follow someone to view their posts...</h1>}
            </div>
            <div className="recommendation">
                    ...
                    <ul>
                        {newestUser.map((user) => {
                            if (user.username !== userInfo.username) {
                                return <li key={user._id}><NavLink className="link text-primary" to={`/u/${user.username}`}>{user.username}</NavLink> (joined {moment(user.createdAt).fromNow()})</li>
                            } else { return null; }
                        })}
                    </ul>
                </div>
                <div className="margin box box-shadow">
                    ...
                    {taggedList.map((tag) => {
                        ...
                        &nbsp;<NavLink to={`/post/${tag._id}`} className="link text-primary">post</NavLink> {moment(tag.createdAt).fromNow()}</p>
                    })}
                </div>
                <div className="margin box box-shadow">
                    <h3 className="box-title">Messages:</h3>
                    {chatList.map((chat) => {
                        return <p key={chat.id}><NavLink to={`/chats?qto=${chat.username}`} className="link text-primary">{chat.username}</NavLink> Messaged you {moment(chat.at).fromNow()}.</p>
                    })}
                </div>
            </div>
        </div>
