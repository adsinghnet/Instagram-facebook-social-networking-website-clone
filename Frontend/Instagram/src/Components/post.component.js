                    ...
                    <div className="box box-shadow">
                        ...
                        {postInfo.hashtags.length > 0 && postInfo.hashtags[0] !== ""
                            ? <div className="post-section">
                                ...
                                {postInfo.hashtags.map((hashtag) => (<span className="text-secondary" key={hashtag}>#{hashtag} </span>))}
                            </div>
                            : null
                        }
                        ...
                        {isOwner ?
                            <div>
                                <h3><NavLink to={`/post/${params.match.params.id}/edit`} className="link"><u>Edit Post</u></NavLink></h3>
                                <h3 className="text-danger link" onClick={DeletePost}><u>Delete Post</u></h3>
                            </div>
                            : null
                        }
                    </div>
                    <div className="margin-top-bottom box box-shadow" id="comment">
                        ...
                        <div className="margin box box-shadow">
                            {comments.map((comment) => {
                                return <p key={comment._id} className="box-text text-muted"><NavLink to={`/u/${commenterName[comment.commenter]}`} className="link text-primary">{commenterName[comment.commenter]}:</NavLink>
                            &nbsp;<span className="text-dark" >{comment.comment}</span> {moment(comment.createdAt).fromNow()}</p>
                            })}
                        </div>
                        {userInfo ?
                            <form onSubmit={addComment}>
                                <textarea className="form-control" rows="5" onChange={({ target: { value } }) => setInputComment(value)}></textarea>
                                <br /> <input className="btn-outline-primary form-control py-4" type="submit" />
                            </form>
                            : null}
                    </div>
                </div>
