...
const Home = () => {
    ...
    const GeneratePost = ({ post }) => {
        return <div key={post._id} className="box box-shadow margin-top-bottom container-sm px-4 w-50">
            <b><NavLink to={`/u/${post.creator}`} className="link fs-2 text text-wrap">{post.creator}</NavLink></b>
            ...
            <div className="post-section">
                {!likeInfo[post._id]
                    ...
                    : <span className="to-like-icon" onClick={() => { UnlikePost(post._id) }}><img src={HeartIcon} alt="Unlike Icon" /></span>}<span>{' '}</span>
            ...
            {post.tags.length > 0 && post.tags[0] !== ""
                ? <div className="post-section ">
                    <p className="form-label"><b>Tags:</b></p>
                    {post.tags.map((tag) => (<span key={tag}><NavLink to={`/u/${tag.toLowerCase()}`} className="link text-primary">@{tag.toLowerCase()} </NavLink></span>))}
                </div>
                : null}
            {post.hashtags.length > 0 && post.hashtags[0] !== ""
                ? <div className="post-section">
                    <p className="form-label"><b>Hashtags:</b></p>
                    {post.hashtags.map((hashtag) => (<span className="text-secondary" key={hashtag}>#{hashtag} </span>))}
                </div>
                : null
            }
        ...
        <div className="container">
            <div className="home">
                {posts.length !== 0 ? (
                    ...
                    : <h3>You need to follow someone to view there posts...</h3>}
            </div>
            <div className="recommendation">
                    ...
                    <ul>
                        {newestUser.map((user) => {
                            if (user.username !== userInfo.username) {
                                return <li key={user._id}><NavLink className="link text-primary" to={`/u/${user.username}`}>{user.username}</NavLink> <span className="text-muted" > (joined {moment(user.createdAt).fromNow()})</span></li>
                            } else { return null; }
                        })}
                    </ul>
                </div>
                <div className="margin box box-shadow">
                    ...
                    {taggedList.map((tag) => {
                        return <p key={tag._id}><NavLink className="link text-primary" to={`/u/${tag.username}`}>{tag.username}</NavLink> tagged you on a
                        &nbsp;<NavLink to={`/post/${tag._id}`} className="link text-primary">post</NavLink> <span className="text-muted" >{moment(tag.createdAt).fromNow()}</span></p>
                    })}
                </div>
                ...
                <div className="margin box box-shadow">
                    ...
                    {chatList.map((chat) => {
                        return <p key={chat.id}><NavLink to={`/chats?qto=${chat.username}`} className="link text-primary">{chat.username}</NavLink> Messaged you <span className="text-muted" >{moment(chat.at).fromNow()}.</span></p>
                    })}
